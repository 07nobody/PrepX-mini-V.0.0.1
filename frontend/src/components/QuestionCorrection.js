...
const QuestionCorrection = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await axios.get('/api/questions/needs-correction');
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Question Correction</h1>
      {questions.length > 0 ? (
        <ul>
        ...
        <p>No questions available for correction.</p>
      )}
