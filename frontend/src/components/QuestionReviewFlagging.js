...
const QuestionReviewFlagging = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await axios.get('/api/questions/review');
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-2xl font-bold mb-4">Question Review and Flagging</h1>
      {questions.length > 0 ? (
        <ul>
        ...
        <p>No questions available for review and flagging.</p>
      )}
...
export default QuestionReviewFlagging;
